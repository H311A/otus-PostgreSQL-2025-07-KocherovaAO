...
### Могут ли две транзакции, выполняющие единственную команду UPDATE одной и той же таблицы (без where), заблокировать друг друга + задание со звёздочкой.
Да, могут. Две транзакции, выполняющие UPDATE всей таблицы могут заблокировать друг друга в следующих случаях:
1. Если в таблице есть индексы, или если планировщик запросов решил использовать разные планы выполнения (например, из-за параллельного чтения данных), одна транзакция может начать обновлять строки с начала таблицы, а другая - с конца.
...
Процесс 75666 ожидает в режиме ShareLock блокировку "транзакция 273536"; заблокирован процессом 75277.
ПОДСКАЗКА:  Подробности запроса смотрите в протоколе сервера.
КОНТЕКСТ:  при изменении кортежа (0,1) в отношении "deadlock_test"
```
Взаимоблокировку удалось воспроизвести. Две транзакции, выполняющие команду UPDATE на одной таблице без условия WHERE, действительно могут заблокировать друг друга. Хотя команда UPDATE всей таблицы выглядит как атомарная операция, механизм ее выполнения (последовательное сканирование и блокировка строк) может при определенных условиях привести к классической взаимоблокировке.
